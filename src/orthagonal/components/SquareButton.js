...
function SquareButton({
  ...
  return (
    <div className={`col justify-between items-center ${className}`}>
      {label && <TextLabel className={`uppercase ${labelClassName}`}>{label}</TextLabel>}
      <span className="w-[35px] h-[35px] col justify-center items-center bg-[#4c4c4c] border border-black rounded">
